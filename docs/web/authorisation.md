...
## Controlled Multi-Account Matrix

Use at least two dedicated accounts where the engagement permits it. Add a privileged account, separate tenant, or unauthenticated state only when required by the application model.

| Actor | Object or function | Expected result | Observed result |
|---|---|---|---|
| Unauthenticated | Public and protected route | Public only | |
| User A | User A object | Read or modify as designed | |
| User A | User B object | Denied | |
| Same-role User A | Administrative function | Denied unless delegated | |
| Tenant A user | Tenant B object or function | Denied | |
| Administrator | Approved administrative function | Allowed and audited | |

Test web, API, background, export, download, and alternate-version paths. Keep authentication, object access, function access, tenant isolation, administrative rights, and execution as separate columns. A valid login proves identity only; it does not prove access to an object, permission to invoke a function, administrative authority, or code execution.

## Burp Comparison Workflow

Capture a baseline request as User A, then repeat the same request as User B or without authentication. Change one object identifier, method, role claim, tenant identifier, or endpoint at a time. Use Autorize or AuthMatrix for controlled comparisons, but verify every interesting result in Repeater with fresh sessions.

Interpret outcomes carefully:

- A `200` response may contain an error, empty object, or cached data rather than authorised access.
- A `403` may be generated by a proxy or WAF; confirm the application decision and repeat it.
- A changed response is not enough; establish whether protected data was disclosed or a state-changing action succeeded.
- A successful object read does not prove update, delete, administrative, or execution rights.

## Validation, Evidence, and Retesting

Use test objects created for the assessment. For reads, record the minimum non-sensitive field proving ownership boundary crossing. For writes or deletes, use reversible test data and confirm the final state with the owner-approved read-back. Document actor, tenant, object, route, method, expected result, actual result, and relevant response evidence with tokens and personal data redacted.

After remediation, rerun the full matrix across web and API paths, including direct requests, alternate methods, exports, background actions, and existing sessions. Link this page with [Authentication](authentication.md), [IDOR and BOLA](idor-bola.md), and [Burp Suite Testing Workflows](burp-suite/workflows.md).
